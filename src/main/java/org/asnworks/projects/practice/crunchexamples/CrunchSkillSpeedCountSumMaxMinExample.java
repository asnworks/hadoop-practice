...
public class CrunchSkillSpeedCountSumMaxMinExample extends Configured implements
		Tool {

	public static void main(String[] args) throws Exception {
		int response = ToolRunner.run(new Configuration(),
				new CrunchSkillSpeedCountSumMaxMinExample(), args);
	...
	public int run(String[] args) throws Exception {
		...
		PTable<String, Double> table = lines.parallelDo(
				new SplitKeyValueDoFn(),
		...
		PTable<String, Double> sum = table.groupByKey().combineValues(
				Aggregators.SUM_DOUBLES());

		System.out.println("Sum of each value for the Key : " + sum.toString());

		PTable<String, Double> max = table.groupByKey().combineValues(
				Aggregators.MAX_DOUBLES());

		System.out.println("Max of each value for the Key : " + max.toString());

		PTable<String, Double> min = table.groupByKey().combineValues(
				Aggregators.MIN_DOUBLES());

		System.out.println("Min of each value for the Key : " + min.toString());

		PTable<String, Long> count = Aggregate.count(table.parallelDo(
				new FindCountDoFn(), Writables.strings()));
